...
public class HadoopTableOperations implements TableOperations {
  ...
  public void commit(TableMetadata base, TableMetadata metadata) {
    ...
    try {
      ...
      validate(useObjectStore, previousVersionsMax, nextVersion, fs, finalMetadataFile);
  ...
  private void validate(
    ...
    if (!useObjectStore) {
      ...
      cleanAllDirtyCommit(fs, previousVersionsMax);
    }
  }

  public void fastFailIfDirtyCommit(
    ...
    if ((currentMaxVersion - nextVersion) > previousVersionsMax && fs.exists(finalMetadataFile)) {
      ...
      throw new CommitFailedException(
          "The commit for version [%s] has actually failed, because the latest version is now an updated to [%s].Are there other clients running in parallel with the current task?",
          nextVersion, currentMaxVersion);
    }
  }

  public void cleanAllDirtyCommit(FileSystem fs, int previousVersionsMax) throws IOException {
    FileStatus[] files =
        fs.listStatus(metadataRoot(), name -> VERSION_PATTERN.matcher(name.getName()).matches());
    List<Path> dirtyCommit = Lists.newArrayList();
    int currentMaxVersion = findVersionWithOutVersionHint(fs);
    ...
    // todo:Perhaps this threshold needs a configuration,But that's not necessary, at least not yet.
